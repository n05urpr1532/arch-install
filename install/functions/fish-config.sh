...
configure_fish_prompt() {
  ...
  # Install required font for tide
  exec_in_container /usr/bin/su -c 'paru -S --noconfirm --needed ttf-meslo-nerd-font-powerlevel10k' - "${user_name}"

  # Install Fisher and Tide
  exec_in_container /usr/bin/su -c 'curl -sL https://git.io/fisher | source && fisher install jorgebucaran/fisher && fisher install IlanCosman/tide@v5' - "${user_name}"
...
if status is-interactive && ! fish_is_root_user
  ...
  set -U tide_right_prompt_items status cmd_duration context jobs virtual_env go vi_mode time
