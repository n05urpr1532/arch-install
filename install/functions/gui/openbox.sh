...
install_openbox () {
  ...
  exec_in_container /usr/bin/su -c 'paru -S --noconfirm --needed xorg xorg-apps libxkbcommon-x11 xdg-utils xf86-input-evdev xf86-input-libinput xf86-input-void xf86-video-dummy xf86-video-fbdev xf86-video-intel' - ${user_name}
  exec_in_container /usr/bin/su -c 'paru -S --noconfirm --needed srandrd libva libva-mesa-driver intel-media-driver mesa-demos noto-fonts grub-customizer hwinfo' - ${user_name}
  ...
  exec_in_container /usr/bin/su -c 'paru -S --noconfirm --needed pipewire pipewire-docs pipewire-alsa pipewire-pulse pipewire-jack pipewire-jack-dropin' - ${user_name}
  exec_in_container /usr/bin/su -c 'paru -S --noconfirm --needed lightdm lightdm-gtk-greeter lightdm-gtk-greeter-settings numlockx accountsservice gnome-keyring libsecret seahorse' - ${user_name}
  # TODO obkey obkey-git lxhotkey-gtk3 ?
  exec_in_container /usr/bin/su -c 'paru -S --noconfirm --needed openbox obconf lxhotkey-gtk3 oblogout-py3-git nitrogen picom rofi network-manager-applet' - ${user_name}
  ...
  exec_in_container /usr/bin/su -c 'paru -S --noconfirm --needed obmenu-generator perl-gtk3 perl-file-desktopentry' - ${user_name}
  ...
  exec_in_container /usr/bin/su -c 'paru -S --noconfirm --needed alacritty xterm geany geany-plugins geany-themes firefox firefox-i18n-fr snapper-gui cpupower-gui' - ${user_name}
